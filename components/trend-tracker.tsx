...
const REGIONS = [
  ...
  },
  {
    ...
    name: "Europe",
    prompt:
      "You are answering from a European perspective, considering EU-based companies, GDPR compliance, and solutions popular in Europe.",
  },
  {
    id: "asia_pacific",
    name: "Asia Pacific",
...
interface AnalysisResult {
  ...
  isDemo?: boolean
  demoReason?: string
  regionPerformance: Array<{ region: string; score?: number; averageScore?: number }>
  personaPerformance: Array<{ persona: string; score?: number; averageScore?: number }>
  competitorMentions: Array<{ brand: string; count: number; avgPosition: number; sentiment: string }>
  sentimentBreakdown: { positive: number; neutral: number; negative: number }
  contextAnalysis: Array<{ context: string; mentions: number; sentiment: string }>
  recommendations: string[]
  linksByBrand: Record<string, Array<{ url: string; brand: string; count: number; title?: string }>>
  deepContextAnalysis?: {
    categories: Array<{ category: string; count: number; sentiment: string; examples: string[] }>
    quotes: Array<{ text: string; category: string; sentiment: string }>
    keywordAssociations: Array<{ keyword: string; count: number; sentiment: string }>
  }
  timestamp: string
  query: string
  brand: string
...
export function TrendTracker({
  ...
  const [queries, setQueries] = useState<string[]>(initialAnalysis?.queries || [""])
  const [competitors, setCompetitors] = useState(initialAnalysis?.competitors?.join(", ") || "")
  const [frequency, setFrequency] = useState<"daily" | "weekly" | "monthly">(initialAnalysis?.frequency || "weekly")
  const [autoRunEnabled, setAutoRunEnabled] = useState(initialAnalysis?.enabled || false)
  ...
  const [selectedPersonas, setSelectedPersonas] = useState<string[]>(PERSONAS.map((p) => p.id)) // State for selected personas
  const [selectedRegions, setSelectedRegions] = useState<string[]>(REGIONS.map((r) => r.id)) // State for selected regions

  // Simulovaná historická data pro trendy
  const [historicalData, setHistoricalData] = useState<Array<{ date: string; score: number }>>([])

  const [showOnboarding, setShowOnboarding] = useState(false)

  useEffect(() => {
    const savedResults = localStorage.getItem("brandvision_analysis_results")
    if (savedResults) {
      try {
        const parsed = JSON.parse(savedResults)
        setResults(parsed)
        console.log("[v0] Loaded saved results:", Object.keys(parsed).length, "queries")
      } catch (error) {
  ...
  useEffect(() => {
    if (Object.keys(results).length > 0) {
      localStorage.setItem("brandvision_analysis_results", JSON.stringify(results))
      console.log("[v0] Saved results to localStorage:", Object.keys(results).length, "queries")
      ...
      const historyData = allResults.map((r) => ({
        ...
        brandMentions: {
          found: r.competitorMentions?.some((c) => c.brand.toLowerCase() === brand.toLowerCase()) || false,
          count: r.competitorMentions?.find((c) => c.brand.toLowerCase() === brand.toLowerCase())?.count || 0,
        },
        model: r.usage?.model || "Unknown",
        timestamp: r.timestamp,
      }))
      localStorage.setItem("analysis_history", JSON.stringify(historyData))

      // Save trend data for historical trends
      const trendData = allResults.map((r, i) => ({
        date: new Date(r.timestamp || Date.now()).toLocaleDateString("cs-CZ", { day: "2-digit", month: "2-digit" }),
        score: r.globalScore,
        mentions: r.competitorMentions?.find((c) => c.brand.toLowerCase() === brand.toLowerCase())?.count || 0,
      }))
  ...
  // API key is now only on server-side in API route
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (costHistory) {
      ...
      const total = history.reduce((sum: number, item: any) => sum + item.cost, 0)
      setTotalCost(total)
    }
  }, [])
  ...
  const runAnalysis = async () => {
    ...
    for (const query of validQueries) {
      try {
        ...
        alert(`Analýza selhala pro dotaz "${query}". Zkontrolujte konzoli pro detaily.`)
    ...
    if (runTotalCost > 0) {
      const costHistory = JSON.parse(localStorage.getItem("brandvision_cost_history") || "[]")
      costHistory.push({
        date: new Date().toISOString(),
        cost: runTotalCost,
        queries: validQueries.length, // Count valid queries
      })
      localStorage.setItem("brandvision_cost_history", JSON.stringify(costHistory))
  ...
  const calculateTrend = () => {
    ...
    const recent = historicalData.slice(-7).reduce((sum, d) => sum + d.score, 0) / 7
    const previous = historicalData.slice(-14, -7).reduce((sum, d) => sum + d.score, 0) / 7
  ...
  const aggregateResults = () => {
    ...
    const scores = allResults.map((r) => r.globalScore).filter((s) => typeof s === "number" && !isNaN(s))
  ...
  const competitorIntelligenceData = latestResult?.competitorMentions
    ? latestResult.competitorMentions.map((comp) => ({
        ...
        shareOfVoice: (comp.count / latestResult.competitorMentions.reduce((sum, c) => sum + c.count, 0)) * 100,
        strengths:
          comp.brand.toLowerCase() === brand.toLowerCase()
            ? ["Silná viditelnost v B2B segmentu", "Pozitivní sentiment", "Kvalitní dokumentace"]
            : ["Etablovaná značka", "Rozsáhlá funkcionalita"],
        weaknesses:
          comp.brand.toLowerCase() === brand.toLowerCase()
            ? ["Nižší penetrace v LATAM", "Méně zmínek než top konkurence"]
        ...
        <div className="mb-6 flex items-center justify-between">
          ...
          <div className="flex gap-3 flex-wrap">
            <div className="text-sm text-gray-600">
              <span className="font-medium">Celkové náklady:</span> ${totalCost.toFixed(4)}
            </div>
            {estimatedMonthlyCost > 0 && (
              <div className="text-sm text-blue-600">
                <span className="font-medium">Měsíční odhad:</span> ${estimatedMonthlyCost.toFixed(2)}
              </div>
            )}
            {aggregated && (
          ...
          <div className="lg:col-span-1 space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Nastavte parametry pro sledování viditelnosti</CardDescription>
              </CardHeader>
              <CardContent>
                <AnalysisConfigForm
                  apiKey={apiKey}
                ...
                <Button
                  onClick={runAnalysis}
                  disabled={loading || queries.length === 0 || !brand || !apiKeyStored}
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <Card className="bg-blue-50 border-blue-200">
              ...
              <CardContent className="text-sm text-blue-800 space-y-2">
                <p>
                  <strong>Skóre 0-100</strong> se vypočítá na základě těchto faktorů:
                </p>
                <ul className="list-disc list-inside space-y-1 ml-4">
                  <li>
                    <strong>Počet zmínek (0-40 bodů):</strong> Každá zmínka vašeho brandu = 20 bodů
                  </li>
                  <li>
                    <strong>Pozice v odpovědi (0-30 bodů):</strong> Čím dříve je brand zmíněn, tím lépe
                    <ul className="list-circle list-inside ml-6 text-xs mt-1">
                      <li>První třetina odpovědi: +30 bodů</li>
                      <li>Prostřední třetina: +20 bodů</li>
                      <li>Poslední třetina: +10 bodů</li>
                    </ul>
                  </li>
                  <li>
                    <strong>Sentiment (0-20 bodů):</strong> Pozitivní zmínka +20, neutrální +10, negativní 0
                  </li>
                  <li>
                    <strong>Srovnání s konkurencí (0-10 bodů):</strong> Bonus pokud jste zmíněn mezi konkurenty
                  </li>
                </ul>
                <div className="mt-3 pt-3 border-t border-blue-300">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-2">
                    <div className="text-xs">
                      <span className="font-semibold">80-100:</span> Výborná viditelnost
                    </div>
                    <div className="text-xs">
                      <span className="font-semibold">60-79:</span> Dobrá viditelnost
                    </div>
                    <div className="text-xs">
                      <span className="font-semibold">40-59:</span> Průměrná viditelnost
                    </div>
                    <div className="text-xs">
                      <span className="font-semibold">0-39:</span> Nízká viditelnost
                    </div>
                  </div>
                </div>
              </CardContent>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  <Card>
                    <CardHeader className="pb-2">
                      <div className="flex items-center gap-2">
                        <CardTitle>Globální skóre viditelnosti</CardTitle>
                        <MetricTooltip
                    ...
                    <CardContent>
                      <div className="text-3xl font-bold text-blue-600">
                        {latestResult.globalScore && !isNaN(latestResult.globalScore)
                          ? Math.round(latestResult.globalScore)
                          : 0}
                      </div>
                      {trend && (
                        <div className="flex items-center mt-2 text-sm">
                          {trend.direction === "up" && <TrendingUp className="h-4 w-4 text-green-600 mr-1" />}
                          {trend.direction === "down" && <TrendingDown className="h-4 w-4 text-red-600 mr-1" />}
                          {trend.direction === "stable" && <Minus className="h-4 w-4 text-gray-600 mr-1" />}
                          <span
                            className={
                              trend.direction === "up"
                                ? "text-green-600"
                  ...
                  {queries.length > 1 && aggregated && (
                    <>
                      <Card>
                        <CardHeader className="pb-2">
                          <CardTitle className="text-sm font-medium text-gray-600">Průměrné skóre</CardTitle>
                        </CardHeader>
                        <CardContent>
                          <div className="text-3xl font-bold text-gray-900">
                            {aggregated.avgScore && !isNaN(aggregated.avgScore) ? Math.round(aggregated.avgScore) : 0}
                          </div>
                          <p className="text-xs text-gray-500 mt-1">Napříč {aggregated.count} dotazy</p>
                        </CardContent>
                      </Card>

                      <Card>
                        <CardHeader className="pb-2">
                          <CardTitle className="text-sm font-medium text-gray-600">Nejlepší výsledek</CardTitle>
                        </CardHeader>
                        <CardContent>
                          <div className="text-3xl font-bold text-green-600">
                            {aggregated.maxScore && !isNaN(aggregated.maxScore) ? Math.round(aggregated.maxScore) : 0}
                          </div>
                        </CardContent>
                      </Card>
                      ...
                      <Card>
                        <CardHeader className="pb-2">
                          <CardTitle className="text-sm font-medium text-gray-600">Nejhorší výsledek</CardTitle>
                        </CardHeader>
                        <CardContent>
                          <div className="text-3xl font-bold text-red-600">
                            {aggregated.minScore && !isNaN(aggregated.minScore) ? Math.round(aggregated.minScore) : 0}
                          </div>
                        </CardContent>
                      </Card>
                    </>
                ...
                {latestResult && (
                  <AdvancedVisualizations
                    data={{
                      brandName: brand,
                      competitors: latestResult.competitorMentions,
                      regionPerformance: latestResult.regionPerformance.map((r) => ({
                        region: r.region,
                        score: r.score || r.averageScore || 0,
                      })),
                      personaPerformance: latestResult.personaPerformance.map((p) => ({
                        persona: p.persona,
                        score: p.score || p.averageScore || 0,
                      })),
                      historicalData,
                    }}
                  />
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div>
                          <div className="text-sm text-gray-600">Input tokeny</div>
                          <div className="text-lg font-semibold">{latestResult.usage.inputTokens.toLocaleString()}</div>
                        </div>
                        <div>
                          <div className="text-sm text-gray-600">Output tokeny</div>
                        ...
                        <div>
                          ...
                          <div className="text-sm font-medium">{latestResult.usage.model}</div>
                        </div>
                        <div>
                          <div className="text-sm text-gray-600">Náklady</div>
                          <div className="text-lg font-semibold text-green-600">
                            ${latestResult.usage.cost.toFixed(4)}
                          </div>
                        </div>
                      </div>
                    </CardContent>
                ...
                {queries.length > 1 && (
                  <Card>
                    <CardHeader>
                      <CardTitle>Rozbor jednotlivých dotazů</CardTitle>
                      <CardDescription>Detailní výsledky pro každý testovaný dotaz</CardDescription>
                    </CardHeader>
                    <CardContent>
                      <Accordion type="single" collapsible className="w-full">
                        {queryResults.map(({ query, result }) => (
                          <AccordionItem key={query} value={query}>
                            <AccordionTrigger>
                              <div className="flex items-center justify-between w-full pr-4">
                                <span className="text-left font-medium">{query}</span>
                                <Badge variant={result.globalScore >= 70 ? "default" : "secondary"}>
                                  Skóre: {Math.round(result.globalScore)}
                                </Badge>
                              </div>
                            </AccordionTrigger>
                            <AccordionContent>
                              <div className="space-y-4 pt-4">
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                  <div>
                                    <div className="text-sm font-medium text-gray-600">Celkové skóre</div>
                                    <div className="text-2xl font-bold text-blue-600">
                                      {Math.round(result.globalScore)}
                                    </div>
                                  </div>
                                  <div>
                                    <div className="text-sm font-medium text-gray-600">Pozitivní sentiment</div>
                                    <div className="text-2xl font-bold text-green-600">
                                      {result.sentimentBreakdown?.positive || 0}%
                                    </div>
                                  </div>
                                  <div>
                                    <div className="text-sm font-medium text-gray-600">Konkurenční zmínky</div>
                                    <div className="text-2xl font-bold text-gray-900">
                                      {(result.competitorMentions || []).length}
                                    </div>
                                  </div>
                                </div>

                                <div>
                                  <h4 className="font-medium mb-2">Top 3 konkurenti v této odpovědi:</h4>
                                  <div className="space-y-2">
                                    {(result.competitorMentions || []).slice(0, 3).map((competitor) => (
                                      <div
                                      ...
                                      >
                                        ...
                                        <div className="flex items-center gap-4 text-sm">
                                          <span className="text-gray-600">{competitor.count} zmínek</span>
                ...
                <Card>
                  <CardHeader>
                    <CardTitle>Trend viditelnosti v čase</CardTitle>
                    <CardDescription>Vývoj skóre za posledních 30 dní (demo data)</CardDescription>
                  </CardHeader>
                  <CardContent>
                    ...
                    <CardHeader>
                      <CardTitle className="flex items-center justify-between">
                        ...
                        <PromptDisplayModal
                          ...
                          userPrompt={`${queries[0] || "User query"}`}
                          context={{ region: "various", persona: "mixed" }}
                        />
                      </CardTitle>
                    </CardHeader>
                    <CardContent>
                      <ResponsiveContainer width="100%" height={250}>
                        <BarChart data={latestResult.regionPerformance}>
                          <CartesianGrid strokeDasharray="3 3" />
                          <XAxis dataKey="region" />
                          <YAxis domain={[0, 100]} />
                          <Tooltip />
                          <Bar dataKey="score" fill="#3b82f6" name="Skóre" />
                        </BarChart>
                      </ResponsiveContainer>
                    </CardContent>
                  </Card>
                    ...
                    <CardHeader>
                      <CardTitle className="flex items-center justify-between">
                        ...
                        <PromptDisplayModal
                          ...
                          userPrompt={`${queries[0] || "User query"}`}
                          context={{ region: "mixed", persona: "various" }}
                        />
                      </CardTitle>
                    </CardHeader>
                    <CardContent>
                      <ResponsiveContainer width="100%" height={250}>
                        <BarChart data={latestResult.personaPerformance}>
                          <CartesianGrid strokeDasharray="3 3" />
                          <XAxis dataKey="persona" angle={-45} textAnchor="end" height={100} />
                          <YAxis domain={[0, 100]} />
                          <Tooltip />
                          <Bar dataKey="score" fill="#10b981" name="Skóre" />
                        </BarChart>
                      </ResponsiveContainer>
                    </CardContent>
                  </Card>
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="flex items-center justify-between">
                      <span>V jakém kontextu je váš brand zmiňován?</span>
                      <PromptDisplayModal
                    ...
                    <CardDescription>Analýza toho, jak AI popisuje váš brand v odpovědích</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
                      {(latestResult.contextAnalysis || []).map((ctx, idx) => (
                        <div key={idx} className="border rounded-lg p-4">
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              <p className="text-gray-900">{ctx.context}</p>
                              <div className="flex items-center gap-3 mt-2">
                                <span className="text-sm text-gray-600">
                                  {ctx.mentions}{" "}
                                  {ctx.mentions === 1 ? "zmínka" : ctx.mentions < 5 ? "zmínky" : "zmínek"}
                                </span>
                                <Badge
                                  variant={
                    ...
                    {latestResult.sentimentBreakdown ? (
                      <div className="grid grid-cols-3 gap-4">
                        <div className="text-center p-4 bg-green-50 rounded-lg">
                          <div className="text-3xl font-bold text-green-600">
                            {latestResult.sentimentBreakdown.positive}%
                          </div>
                          <div className="text-sm text-green-800 mt-1">Pozitivní</div>
                        </div>
                        <div className="text-center p-4 bg-gray-50 rounded-lg">
                          <div className="text-3xl font-bold text-gray-600">
                            {latestResult.sentimentBreakdown.neutral}%
                          </div>
                          <div className="text-sm text-gray-800 mt-1">Neutrální</div>
                        </div>
                        <div className="text-center p-4 bg-red-50 rounded-lg">
                          <div className="text-3xl font-bold text-red-600">
                            {latestResult.sentimentBreakdown.negative}%
                          </div>
                          <div className="text-sm text-red-800 mt-1">Negativní</div>
                        </div>
                      </div>
                    ) : (
                      <p className="text-gray-500 text-center">Data nejsou k dispozici</p>
                    )}
                  </CardContent>
                </Card>
                ...
                {competitorRanking.length > 0 && (
                  <Card className="mb-6">
                    <CardHeader>
                      <CardTitle className="flex items-center justify-between">
                        <span>Žebříček brandů v AI odpovědích</span>
                        <PromptDisplayModal
                    ...
                    <CardContent>
                          ...
                          >
                            <div className="flex items-center gap-4 flex-1">
                              <div className="w-12 text-center">
                                <div className="text-2xl font-bold text-gray-700">#{comp.rank}</div>
                                {comp.rankChange !== 0 && (
                                  <div
                              ...
                              <div className="flex-1">
                                ...
                                <div className="text-sm text-gray-600">
                                  {comp.count} zmínek · Průměrná pozice: {comp.avgPosition} · Sentiment:{" "}
                                  {comp.sentiment}
                                </div>
                              </div>
                              ...
                              <div className="text-right">
                                <div className="text-sm font-medium text-gray-700">
                                  {Math.round(
                                    (comp.count / competitorRanking.reduce((sum, c) => sum + c.count, 0)) * 100,
                                  )}
                                  %
                                </div>
                                <div className="text-xs text-gray-500">podíl zmínek</div>
                              </div>
                            </div>
                          </div>
                        ))}
                      ...
                      <div className="mt-4 p-3 bg-gray-50 rounded-lg text-xs text-gray-600">
                        <strong>Vysvětlení:</strong> Žebříček je sestaven podle počtu zmínek a průměrné pozice v
                        odpovědích AI. Šipky ukazují změnu pozice oproti minulému měsíci.
                      </div>
                    </CardContent>
                  </Card>
                )}
                ...
                {/* Analýza odkazů */}
                {latestResult?.linksByBrand && Object.keys(latestResult.linksByBrand).length > 0 && (
                  <Card className="p-6">
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="text-xl font-semibold">Analýza odkazů v AI odpovědích</h3>
                      <PromptDisplayModal
                    ...
                    <p className="text-sm text-gray-600 mb-6">
                      Nejčastější URL adresy, na které AI odpovědi odkazují, rozdělené podle brandů.
                    </p>

                    <div className="space-y-6">
                      {Object.entries(latestResult.linksByBrand).map(([brandName, links]) => {
                        const isMyBrand = brandName.toLowerCase() === brand.toLowerCase()
                        const totalLinks = (links as any[]).reduce((sum, link) => sum + link.count, 0)

                        return (
                          <div
                            key={brandName}
                            className={`p-4 rounded-lg border-2 ${isMyBrand ? "border-blue-500 bg-blue-50" : "border-gray-200 bg-gray-50"}`}
                          >
                            <div className="flex items-center justify-between mb-3">
                              <div className="flex items-center gap-2">
                                <h4 className="font-semibold text-lg">{brandName}</h4>
                                {isMyBrand && (
                                  <span className="text-xs bg-blue-500 text-white px-2 py-1 rounded">Váš brand</span>
                                )}
                              </div>
                              <div className="text-sm text-gray-600">
                                Celkem zmínek: <span className="font-semibold">{totalLinks}</span>
                              </div>
                            </div>

                            <div className="space-y-2">
                              {(links as any[]).slice(0, 5).map((link, idx) => (
                                <div
                                  key={idx}
                                ...
                                >
                                  ...
                                  <div className="ml-4 flex items-center gap-2">
                                    <span className="text-xs text-gray-500">{link.count}× zmíněno</span>
                                    <div className="w-16 h-2 bg-gray-200 rounded-full overflow-hidden">
                                      <div
                                        className="h-full bg-blue-500"
                                        style={{ width: `${(link.count / totalLinks) * 100}%` }}
                                      />
                                    </div>
                                  </div>
                                </div>
                            ...
                            {(links as any[]).length > 5 && (
                              <p className="text-xs text-gray-500 mt-2 text-center">
                                ... a dalších {(links as any[]).length - 5} odkazů
                              </p>
                            )}
                          </div>
                        )
                    ...
                    <div className="mt-6 p-4 bg-gray-50 rounded-lg">
                      <h4 className="font-semibold mb-2">Souhrn</h4>
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        <div>
                          <span className="text-gray-600">Vaše odkazy:</span>
                          <span className="ml-2 font-semibold">
                            {((latestResult.linksByBrand[brand] as any[]) || []).reduce(
                        ...
                        <div>
                          <span className="text-gray-600">Konkurenční odkazy:</span>
                          <span className="ml-2 font-semibold">
                            {Object.entries(latestResult.linksByBrand)
                              .filter(([b]) => b.toLowerCase() !== brand.toLowerCase())
                              .reduce((sum, [, links]) => sum + (links as any[]).reduce((s, l) => s + l.count, 0), 0)}
                          </span>
                        </div>
                      </div>
                    </div>
                ...
                <Card>
                  <CardHeader>
                    <CardTitle>Strategická doporučení</CardTitle>
                  </CardHeader>
                  <CardContent>
                    {latestResult.recommendations && latestResult.recommendations.length > 0 ? (
                      ...
                      <p className="text-gray-500 text-center">Doporučení nejsou k dispozici</p>
                    )}
                  </CardContent>
                </Card>
                ...
                {latestResult?.deepContextAnalysis && (
                  <DeepContextAnalysis
                    brandName={brand}
                    mentions={latestResult.deepContextAnalysis.categories}
                    quotes={latestResult.deepContextAnalysis.quotes}
                  />
                )}
                ...
                <ActionItems
                  ...
                  competitorAverage={
                    latestResult.competitorMentions?.length > 0
                      ? latestResult.competitorMentions.reduce((sum: number, c: any) => sum + (c.score || 0), 0) /
                        latestResult.competitorMentions.length
                      : 0
                  }
                  regionPerformance={latestResult.regionPerformance || []}
                  personaPerformance={latestResult.personaPerformance || []}
                />

                {/* CHANGE: Added ExportManager component before closing div */}
                <ExportManager analysisData={latestResult} brandName={brand} />
              </div>
            )}
          </div>
        </div>
